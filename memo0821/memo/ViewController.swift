...
let cp = CGSize(width: 500, height: 500)
...
class ViewController: UIViewController {
    ...
    @objc func saveImage(_ image: UIImage, didFinishSavingWithError error: NSError?, contextInfo: UnsafeRawPointer) {
        if let error = error {
            ...
            let ac = UIAlertController(title: "Saved", message: "저장되었습니다.", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "OK", style: .default))
            present(ac, animated: true)
        }
    }
...
extension UIView {
    ...
    func takeScreenshot() -> UIImage {
        ...
        UIGraphicsBeginImageContextWithOptions(cp, false, UIScreen.main.scale)
